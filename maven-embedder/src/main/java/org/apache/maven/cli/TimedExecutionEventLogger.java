 ...
 */
public class TimedExecutionEventLogger
    extends AbstractExecutionListener
{
    ...
    private final Map<MojoExecution, Long> mojoStartDates = new ConcurrentHashMap<MojoExecution, Long>();
    private final Map<MojoExecution, Long> mojoDurations = new ConcurrentHashMap<MojoExecution, Long>();

    public TimedExecutionEventLogger( Logger logger )
    {
        if ( logger == null )
        {
    ...
    private void logStats( MavenSession session )
    {
        ...
        for ( Entry<MojoExecution, Long> entry : mojoDurations.entrySet() )
        {
        	StringBuilder buffer = new StringBuilder( 128 );

        	append( buffer, entry.getKey() );

        	buffer.append( ": " );

        	buffer.append( getFormattedTime( entry.getValue() ) );
    ...
    {
    	mojoStartDates.put( event.getMojoExecution(), System.currentTimeMillis() );
    ...
    {
    	Long start = mojoStartDates.remove( event.getMojoExecution() );
    	mojoDurations.put( event.getMojoExecution(), System.currentTimeMillis() - start );
    }
